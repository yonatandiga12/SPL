...
public abstract class MicroService implements Runnable {
    ...
    protected LinkedBlockingDeque<Message> ticksQueue;
    protected LinkedBlockingDeque<Message> otherMsgQueue;

    protected ConcurrentHashMap< Class<? extends Message>, Callback > callbacksQueue = new ConcurrentHashMap<>();;
    protected boolean iAmBusy = false;
    ...
    public MicroService(String name) {
        ...
        ticksQueue = new LinkedBlockingDeque<>();
        otherMsgQueue = new LinkedBlockingDeque<>();
    }
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        callbacksQueue.put(type,  callback);
    ...
    protected final void terminate() throws InterruptedException {
        checkPublishConference();
        this.terminated = true;
    }

    protected  void checkPublishConference(){
        if(!otherMsgQueue.isEmpty() && this instanceof StudentService){
            for(Message message : otherMsgQueue){
                if(message instanceof PublishConferenceBroadcast){
                    callbacksQueue.get(message.getClass()).call(message);
                }
            }
        }
    }
    ...
    public final void run() {
        if(!messageBus.isMicroServiceRegisterd(this))
            messageBus.register(this);
        initialize();
        if(!(this instanceof TimeService)) {
            while (!terminated) {
                try {
                    Message curr = messageBus.awaitMessage(this);
                    if (curr instanceof StopBroadcast) {
                        Thread.currentThread().interrupt();
                        terminate();
                    } else {
                        if (curr instanceof TickBroadcast) {
                            callbacksQueue.get(curr.getClass()).call(curr);
                        } else {
                            otherMsgQueue.push(curr);
                            if (!iAmBusy) {
                                callbacksQueue.get(curr.getClass()).call(curr);
                                otherMsgQueue.remove(curr);
                            }
                        }
                    }
    ...
    //CHECKS IF IM IN THE MIDDLE OF ANOTHER EVENT
    private void checkIfImBusy(Message curr) {
        if(!iAmBusy){
            Message next = otherMsgQueue.getFirst();
            callbacksQueue.get(next.getClass()).call(next);
        }
    }
}
